...
class TraditionalEMA:
    @torch.no_grad()
    def __init__(self, net, halflife_img=2**20, rampup_ratio=0.3):
        self.net = net
        self.halflife_img = halflife_img
    ...
    @torch.no_grad()
    def update(self, cur_nimg, batch_size):
        halflife_img = self.halflife_img
        if self.rampup_ratio is not None:
            halflife_img = min(halflife_img, cur_nimg / self.rampup_ratio)
        beta = 0.5 ** (batch_size / max(halflife_img, 1e-8))
        for p_net, p_ema in zip(self.net.parameters(), self.ema.parameters()):
            p_ema.lerp_(p_net, 1 - beta)
